...
function startPanelServer(port, sansekai, getSock) {
    const server = http.createServer(async (req, res) => {
        ...
        if (method === "GET" && parsedUrl.pathname === "/api/models") {
            try {
                ...
                let settings = { primaryModel: "google/gemini-2.5-flash-lite" };
        ...
        if (method === "GET" && parsedUrl.pathname === "/api/history") {
            try {
                ...
                const history = safeHistory.map(m => {
                    ...
                    if (m.role === "user" && typeof content === "string" && content.includes("MENSAGEM:")) {
            ...
            req.on("end", async () => {
                try {
                        ...
                        if (fileData.forwardToWhatsapp) {
                            ...
                            if (sock) {
                                try {
                                    ...
                                    const owners = (config.OWNER_NUMBERS || ["551420370026"]).map(num => num.includes('@') ? num : num + '@s.whatsapp.net');
                    ...
                    const aiResult = await sansekai.bochecha._callAI({
                        ...
                        pushname: "Marcos",
                        sender: ((config.OWNER_NUMBERS && config.OWNER_NUMBERS[0]) || "551420370026") + "@s.whatsapp.net",
                        prompt: finalPrompt,
                        isOwner: true,
                        sock: sock,
                        messageRef: null
                    ...
                    // Salva a resposta da IA no histórico para manter contexto correto na próxima mensagem
                    if (aiResult.output) {
                        await sansekai.sessionManager.addMessage("painel-marcos", "assistant", aiResult.output);
                    }
    ...
    server.on('error', (err) => {
        if (err.code === 'EADDRINUSE') {
            console.warn(chalk.yellow(`[⚠️ Painel Web] Porta ${port} já está em uso. Painel ignorado nesta instância.`));
        } else {
            console.error(chalk.red(`[❌ Painel Web] Erro inesperado no servidor:`), err.message);
        }
    });
