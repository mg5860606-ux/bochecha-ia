module.exports = {
    definition: {
        function: {
            name: "antistatus",
            description: "Ativa ou desativa o antistatus no grupo.",
            parameters: {
                type: "object",
                properties: {
    ...
    async execute(args, { sock, from, isOwner, message }) {
        ...
        if (input === "on" || input === "ativar" || input === "true" || input === "1" || input === "sim") {
            ...
            return "Use: */antistatus on* ou */antistatus off*";
        }

        const storage = global.storage || require("../sansekai").storage;
        await storage.updateGroupSecurity(from, "antistatus", estado);

        const status = estado ? "ATIVADO ✅ (Bochecha banirá automaticamente anúncios/links de status!)" : "DESATIVADO ❌";
        await sock.sendMessage(from, { text: `🛡️ Proteção *ANTISTATUS* atualizada!\n\nEstado: ${status}` });
        return `O antistatus foi configurado para ${estado}.`;
    }
};
