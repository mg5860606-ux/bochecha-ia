// Nota: NODE_TLS_REJECT_UNAUTHORIZED removido por segurança.
// Se necessário para ambientes específicos, defina via variável de ambiente do sistema.
...
// Cache global compartilhado para Anti-Delete, Ranking e store
...
// pino já importado acima como 'pino'
...
// Store unificado usando o messageCache global (consolida duplicatas)
const store = {
	bind(ev) {
	...
	async loadMessage(jid, id) {
		return messageCache.get(`${jid}:${id}`) || null;
	}
};
...
async function startBot() {
	...
	if (!state.creds.registered) {
		if (globalUseQRCode === null) {
			...
			console.log(chalk.white(' [3] - Terminal Chat (Falar com a IA)'));
			console.log(chalk.cyan('======================================='));
			...
			if (isServer) {
				...
				const responsePromise = question(chalk.yellow('Digite 1, 2 ou 3 (Padrão 1 em 8s): '));
				...
				if (opcao.trim() === '3') {
					console.log(chalk.green('Iniciando modo Terminal Chat...'));
					const { startTerminalChat } = require('./terminal-chat.js');
					await startTerminalChat();
					return; // Não conecta ao WhatsApp
