module.exports = {
    definition: {
        function: {
            name: "antipagamento",
            description: "Ativa ou desativa o antipagamento no grupo.",
            parameters: {
                type: "object",
                properties: {
    ...
    async execute(args, { sock, from, isOwner, message }) {
        ...
        if (input === "on" || input === "ativar" || input === "true" || input === "1" || input === "sim") {
            ...
            return "Use: */antipagamento on* ou */antipagamento off*";
        }

        const storage = global.storage || require("../sansekai").storage;
        await storage.updateGroupSecurity(from, "antipagamento", estado);

        const status = estado ? "ATIVADO ✅ (Bochecha banirá automaticamente tentativas de pagamento não autorizadas!)" : "DESATIVADO ❌";
        await sock.sendMessage(from, { text: `🛡️ Proteção *ANTIPAGAMENTO* atualizada!\n\nEstado: ${status}` });
        return `O antipagamento foi configurado para ${estado}.`;
    }
};
