module.exports = {
    definition: {
        function: {
            name: "antiflood",
            description: "Ativa ou desativa o antiflood no grupo.",
            parameters: {
                type: "object",
                properties: {
    ...
    async execute(args, { sock, from, isOwner, message }) {
        ...
        if (input === "on" || input === "ativar" || input === "true" || input === "1" || input === "sim") {
            ...
            return "Use: */antiflood on* ou */antiflood off*";
        }

        const storage = global.storage || require("../sansekai").storage;
        await storage.updateGroupSecurity(from, "antiflood", estado);

        const status = estado ? "ATIVADO ✅ (Bochecha banirá automaticamente spammers/flooders!)" : "DESATIVADO ❌";
        await sock.sendMessage(from, { text: `🛡️ Proteção *ANTIFLOOD* atualizada!\n\nEstado: ${status}` });
        return `O antiflood foi configurado para ${estado}.`;
    }
};
