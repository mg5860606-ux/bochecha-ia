...
class LocalMarkdownMemory {
    constructor() {
        ...
        this.globalMemoryFile = path.join(this.memoryDir, 'MEMORY.md');
        if (!fs.existsSync(this.globalMemoryFile)) {
            fs.writeFileSync(this.globalMemoryFile, '# Memória de Longo Prazo da IA\n\nEste arquivo contém os fatos, preferências e lembranças de longo prazo aprendidos pela IA sobre os usuários e o sistema.\n\n## 🧠 Fatos e Preferências Aprendidos\n\n');
        }
    }

    _getTodayDiaryFile() {
        const today = moment().tz("America/Sao_Paulo").format("YYYY-MM-DD");
        return path.join(this.memoryDir, `diario_${today}.md`);
    }

    _getYesterdayDiaryFile() {
        const yesterday = moment().tz("America/Sao_Paulo").subtract(1, 'day').format("YYYY-MM-DD");
        return path.join(this.memoryDir, `diario_${yesterday}.md`);
    }

    async logToDiary(chatName, senderName, senderNum, messageText, role) {
        try {
            const file = this._getTodayDiaryFile();
            const time = moment().tz("America/Sao_Paulo").format("HH:mm:ss");
            const senderTag = role === 'assistant' ? 'Bochecha' : `${senderName} (@${senderNum.split('@')[0]})`;
            const entry = `[${time}] [Chat: "${chatName}"] [${senderTag}]: ${messageText}\n`;
            
            fs.appendFileSync(file, entry, 'utf8');
        } catch (e) {
            console.error("LocalMarkdownMemory.logToDiary", e);
        }
    }

    async learnFact(userName, userNum, factText) {
        try {
            let content = fs.readFileSync(this.globalMemoryFile, 'utf8');
            const cleanNum = userNum.split('@')[0];
            const factEntry = `- [@${cleanNum}] (${userName}): ${factText}`;

            if (content.toLowerCase().includes(factText.toLowerCase().trim())) {
                return;
            }

            content = content.trim() + `\n${factEntry}\n`;
            fs.writeFileSync(this.globalMemoryFile, content, 'utf8');
            console.log(chalk.green(`[🧠 MEMÓRIA INFALLIBLE] Fato aprendido e gravado no MEMORY.md para @${cleanNum}: "${factText}"`));
        } catch (e) {
            console.error("LocalMarkdownMemory.learnFact", e);
        }
    }

    async getLTMContext(userNum) {
        try {
            if (!fs.existsSync(this.globalMemoryFile)) return "";
            const content = fs.readFileSync(this.globalMemoryFile, 'utf8');
            const cleanNum = userNum.split('@')[0];
            
            const lines = content.split('\n');
            const userFacts = lines.filter(l => l.includes(`@${cleanNum}`));
            
            if (userFacts.length > 0) {
                return `\n\n[MEMÓRIAS DE LONGO PRAZO DO SEU CÉREBRO SOBRE @${cleanNum} (LTM - MEMORY.md)]:\n` + 
                    userFacts.join('\n');
            }
        } catch (e) {
            console.error("LocalMarkdownMemory.getLTMContext", e);
        }
        return "";
    }

    async getDiariesContext() {
        let diariesContext = "";
        try {
            const todayFile = this._getTodayDiaryFile();
            const yesterdayFile = this._getYesterdayDiaryFile();

            if (fs.existsSync(yesterdayFile)) {
                const yContent = fs.readFileSync(yesterdayFile, 'utf8');
                const lastLines = yContent.split('\n').slice(-30).join('\n');
                diariesContext += `\n\n[DIÁRIO DE CONVERSAS DE ONTEM (Últimas 30 mensagens)]:\n${lastLines}`;
            }

            if (fs.existsSync(todayFile)) {
                const tContent = fs.readFileSync(todayFile, 'utf8');
                const lastLines = tContent.split('\n').slice(-30).join('\n');
                diariesContext += `\n\n[DIÁRIO DE CONVERSAS DE HOJE (Últimas 30 mensagens)]:\n${lastLines}`;
            }
        } catch (e) {
            console.error("LocalMarkdownMemory.getDiariesContext", e);
        }
        return diariesContext;
    }
}

const localMemory = new LocalMarkdownMemory();
global.localMemory = localMemory;
...
class StorageManager {
    ...
    async read(filePath, defaultValue = {}) {
        ...
        try {
            ...
            if (!existsLocal) {
                // Se o arquivo local não existe, tenta puxar do Firebase Firestore se ativo
                ...
                if (!cloudLoaded) {
                    ...
                    // Salva backup inicial assincronamente no Firebase se ativo
                    if (db) {
                        const docRef = doc(db, "database_json", baseName);
                        setDoc(docRef, {
                            data: defaultValue,
    ...
    async write(filePath, data) {
        ...
        try {
            ...
            // Escrita assíncrona não-bloqueante no Firestore se ativo
            const baseName = path.basename(filePath);
                    ...
                    if (cleanFacts.length > 0) {
                        ...
                        for (const fact of cleanFacts) {
                            ...
                            if (!exists) {
                                ...
                                await localMemory.learnFact("Membro", userId, fact);
                            }
                        }
...
class PromptComposer {
    ...
    async build(chatId, isOwner, userData = {}) {
        ...
        // Recupera fatos semânticos e diários locais do MEMORY.md / Diários Markdown
        try {
            const localLTM = await localMemory.getLTMContext(userData.userId);
            if (localLTM) {
                context += localLTM;
            }
            const localDiaries = await localMemory.getDiariesContext();
            if (localDiaries) {
                context += localDiaries;
            }
        } catch (memErr) {
            Logger.error("PromptComposer.LocalMemoryFetch", memErr);
        }
            ...
            if (!parsedMessage.key.fromMe && (body || hasMedia)) {
                ...
                if (!isCommand) {
                    try {
                        ...
                        await localMemory.logToDiary(logGroupName, pushname, rawSender, body || "(Mídia/Imagem/Vídeo)", 'user');
                    } catch (histErr) {
...
// ═══════════════════════════════════════════════════════
    ...
    async _callAI({ chatId, pushname, sender, prompt, isOwner, sock, messageRef }) {
        ...
        await localMemory.logToDiary(logGroupName, 'Bochecha', sock.user.id, output, 'assistant');
