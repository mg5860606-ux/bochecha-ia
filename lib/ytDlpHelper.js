...
class YtDlpHelper {
     ...
     */
    static async downloadVideo(url, targetPath, maxDuration = MAX_VIDEO_DURATION_SECONDS) {
        ...
        const maxDurationArg = maxDuration ? `--match-filter "duration <= ${maxDuration}"` : "";
        const noUpdateArg = "--no-update";

        return new Promise((resolve, reject) => {
            const command = `"${YT_DLP_PATH}" ${noUpdateArg} ${maxDurationArg} -f "${formatOpt}" -o "${targetPath}" "${url}"`;
