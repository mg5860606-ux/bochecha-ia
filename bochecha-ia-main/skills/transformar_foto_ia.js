...
module.exports = {
    ...
    async execute(args, { sock, from, message }) {
        ...
        try {
            ...
            // 3. Endpoints de Elite (Token carregado do config ou fallback)
            let CANVAS_TOKEN = "6b37bf08416e08c4276b4d55cc276be2";
            try {
                const cfg = JSON.parse(require('fs').readFileSync(require('path').join(__dirname, '..', 'key.json'), 'utf8'));
                if (cfg.canvasToken) CANVAS_TOKEN = cfg.canvasToken;
            } catch {}
            let endpoint = "";
