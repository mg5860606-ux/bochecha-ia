...
class KeyRotationEngine {
    ...
    _getPrioritizedModels(prompt, tools) {
        // Verifica se tem modelo fixo salvo nas preferências
        try {
            const prefsPath = path.join(__dirname, 'terminal_prefs.json');
            if (fs.existsSync(prefsPath)) {
                const prefs = JSON.parse(fs.readFileSync(prefsPath, 'utf8'));
                if (prefs.fixedModel) {
                    return [prefs.fixedModel];
                }
            }
        } catch {}
...
class DialogSession {
    constructor() {
        this.maxMessages = 300; // Limite de gatilho para sumarização
        this.targetHistoryLength = 100; // Quanto manter intacto após sumarizar
        
        // Inicializa o banco SQLite em memory/chats.db
    ...
    async getHistory(chatId) {
        try {
            const stmt = this.db.prepare('SELECT role, content FROM messages WHERE chatId = ? ORDER BY id ASC');
            const rows = stmt.all(chatId);
            
            // Injeta o resumo no início se ele existir
                ...
                return [
                    {
                        ...
                        summary: summary,
                        role: 'user',
                        content: `[SISTEMA - RESUMO DAS INTERAÇÕES ANTERIORES]: ${summary}`
                    },
                    ...rows
                ];
            }
            ...
            Logger.error('DialogSession.getHistory', e);
            return [];
        }
    }
    ...
    async saveHistory(chatId, history) {
        try {
            const deleteStmt = this.db.prepare('DELETE FROM messages WHERE chatId = ?');
            deleteStmt.run(chatId);
            
            const insertStmt = this.db.prepare('INSERT INTO messages (chatId, role, content) VALUES (?, ?, ?)');
            
            for (const msg of history) {
                if (msg.isSummaryMetadata) {
                    await this.saveSummary(chatId, msg.summary);
                    continue;
                }
                insertStmt.run(chatId, msg.role, msg.content);
            }
        } catch (e) {
            Logger.error('DialogSession.saveHistory', e);
        }
    }

    async getSummary(chatId) {
        try {
            const stmt = this.db.prepare('SELECT summary FROM summaries WHERE chatId = ?');
    ...
    async saveSummary(chatId, summary) {
        try {
            const stmt = this.db.prepare('INSERT OR REPLACE INTO summaries (chatId, summary) VALUES (?, ?)');
            stmt.run(chatId, summary);
        } catch (e) {
            Logger.error('DialogSession.saveSummary', e);
        }
    }
    ...
    async _autoCompress(chatId, history) {
        ...
        const chatLogs = toCompress.map(m => {
            if (m.role === 'model' || m.role === 'assistant') return `[Bochecha]: ${m.content}`;
            const content = m.content || '';
            if (content.startsWith('[De:')) return content;
            return `[Membro]: ${content}`;
        }).join('\n');

        const compactPrompt = `Comprima as conversas a seguir em um resumo denso, objetivo e puramente factual em português brasileiro. IMPORTANTE: Preserve o nome de cada pessoa que falou (ex: "Davy disse X", "João perguntou Y"). Retenha detalhes aprendidos sobre os usuários ativos, piadas locais e o humor geral. Mescle com o resumo anterior caso ele exista.\n\n[Resumo Prévio]: ${currentSummary || "Nenhum"}\n\n[Novas Mensagens a Sumarizar]:\n${chatLogs}`;
        ...
        try {
            ...
            await this.saveSummary(chatId, newSummary);
            
            Logger.success("DialogSession", `Sumarização efetuada! Compresso com sucesso.`);

            await this.saveHistory(chatId, toKeep);
        } catch (e) {
    ...
    async addMessage(chatId, role, content) {
        try {
            ...
            history = await this.getHistory(chatId);
            if (history.length > 0 && history[0].isSummaryMetadata) {
                history.shift();
            }
            ...
            Logger.error('DialogSession.addMessage', e);
        }
    }
}
...
class SecuritySystem {
    ...
    static async handleAudioReactions(sock, chatId, body, msgRef) {
        try {
            ...
            // Apenas triggers com arquivos de áudio que existem no diretório lib/
            const reactions = {
                "menu": "menu.mp3"
            };
    ...
    static async handleAutoReplies(sock, chatId, body, msgRef) {
        try {
            ...
            const db = await storage.read(AUTOREPLY_FILE, {});
...
class SchedulerSystem {
    ...
    static start(sock) {
        ...
        setInterval(async () => {
            ...
            try {
                const db = await storage.read(NOTURNO_FILE, {});
                ...
                for (const [from, config] of Object.entries(db)) {
                    if (nowStr === config.fechar && config.lastAction !== 'fechado') {
                        ...
                        await storage.write(NOTURNO_FILE, db);
...
class PromptComposer {
    ...
    async build(chatId, isOwner, userData = {}) {
        ...
        context += `\n- **REGRA CRÍTICA DE IDENTIDADE**: Cada mensagem que chega tem uma tag [De: Nome (@número)]. Essa tag identifica EXATAMENTE quem está falando com você. NUNCA confunda a identidade do remetente atual com mensagens anteriores de outras pessoas. Se o usuário atual é "Davy", RESPONDA A DAVY e não a outra pessoa mencionada antes. Nunca atribua falas de uma pessoa a outra.`;
...
class BochechaEngine {
    ...
    async triggerReflection(isManual = false) {
        try {
            ...
            // Busca conversas recentes do histórico lendo arquivos do diretório de memória
            let keys = [];
            try {
                const memoryFiles = fs.readdirSync(MEMORY_DIR).filter(f => f.endsWith('.json'));
                keys = memoryFiles.map(f => f.replace('.json', '').replace(/_/g, ':'));
            } catch {}
            if (keys.length === 0) {
                if (isManual) {
...
${chatLogs}`;
    ...
    async handleMessage(upsert, sock, store, parsedMessage) {
        try {
            ...
            if (isGroup && !parsedMessage.key.fromMe) {
                // 1. Salva no cache temporário para Anti-Delete (Expira em 1 hora, com LRU de 500 msgs)
                ...
                // LRU: Remove mensagens mais antigas quando o cache excede 500 entradas
            ...
            if (!parsedMessage.key.fromMe && shouldProcessCognitive) {
                ...
                // 2. Alerta ao dono quando afinidade zera (sem ban automático para evitar falsos positivos)
                if (isGroup && affStats.affinity === 0 && !isOwner) {
                    Logger.warn("Moderation.AffinityAlert", `Afinidade de @${sender} caiu para 0%. Enviando alerta ao dono.`);
                    try {
                        await sock.sendMessage(from, { 
                            text: `⚠️ *ALERTA DE AFINIDADE ZERO* ⚠️\n\nMinha afinidade com @${sender} chegou a *0%*. Tô de olho nesse vacilão, Marcos. Me dá a ordem e eu removo! 🥀`,
                            mentions: [rawSender]
                        });

                        // Alerta privado ao dono ao invés de ban autônomo
                        await BochechaEngine.sendTelemetry(`⚠️ *ALERTA DE AFINIDADE ZERO* ⚠️\n\nA afinidade do participante @${sender} (${pushname}) atingiu *0%* no grupo ${from.split('@')[0]}.\n\nUse */ban* para removê-lo se necessário.`);
                    } catch (err) {
                        Logger.error("Moderation.AffinityAlert", err);
                    }
                }
            ...
            if (isOwner && body.startsWith("/")) {
                ...
                switch (cmd) {
                    ...
                    case "/model": {
                        const prefsPath = path.join(__dirname, 'terminal_prefs.json');
                        const allModels = this.availableModels || keyRotator.availableModels || [];
                        
                        if (arg === '0' || arg.toLowerCase() === 'auto') {
                            try { fs.writeFileSync(prefsPath, JSON.stringify({ fixedModel: null }, null, 2)); } catch {}
                            await parsedMessage.reply('✅ *Modo automático ativado!* O Bochecha vai escolher o melhor modelo disponível.');
                            return;
                        }
                        
                        const num = parseInt(arg);
                        if (num >= 1 && num <= allModels.length) {
                            const chosen = allModels[num - 1];
                            try { fs.writeFileSync(prefsPath, JSON.stringify({ fixedModel: chosen }, null, 2)); } catch {}
                            await parsedMessage.reply(`✅ *Modelo fixado:* ${chosen.split('/').pop().replace(':free', '')}`);
                            return;
                        }

                        let currentModel = 'automático';
                        try {
                            if (fs.existsSync(prefsPath)) {
                                const p = JSON.parse(fs.readFileSync(prefsPath, 'utf8'));
                                if (p.fixedModel) currentModel = p.fixedModel.split('/').pop().replace(':free', '');
                            }
                        } catch {}

                        let list = `🤖 *MODELOS DISPONÍVEIS* 🤖\n\n`;
                        list += `*Atual:* ${currentModel}\n\n`;
                        list += `[0] - Automático\n`;
                        ...
                        list += `\n*Uso:* /modelo 1`;
                        await parsedMessage.reply(list);
                        return;
                    }
                                    ...
                                    if (!/^\d+$/.test(nameToSearch) && nameToSearch.length > 0) { // Apenas se não for um número de telefone puro
                                        ...
                                        try {
                                            ...
                                            if (fs.existsSync(dbPath)) {
                                                ...
                                                // Prioridade 1: Match exato de pushname (previne confusão entre nomes parecidos)
                                                let matchedEntry = entries.find(e => {
                                                    const normPush = normalize(e.pushname);
                                                    return normPush === nameToSearch;
                                                });
                                                
                                                // Prioridade 2: Match exato de número
                                                if (!matchedEntry) {
                                                    matchedEntry = entries.find(e => {
                                                        const normUser = normalize(e.user?.split('@')[0]);
                                                        return normUser === nameToSearch;
                                                    });
                                                }
                                                
                                                // Prioridade 3: Match parcial APENAS se o nome buscado tem 4+ caracteres (evita falsos positivos curtos)
                                                if (!matchedEntry && nameToSearch.length >= 4) {
                                                    matchedEntry = entries.find(e => {
                                                        const normPush = normalize(e.pushname);
                                                        return normPush.includes(nameToSearch) || nameToSearch.includes(normPush);
                                                    });
                                                }
                                        ...
                                        // 2. Se não achou, busca na lista de participantes do grupo (match exato primeiro, depois parcial)
                                        if (!foundJid && isGroup) {
                                            ...
                                            for (const p of participants) {
                                                ...
                                                if (pName === nameToSearch || pNum === nameToSearch) {
                                                    foundJid = p.id;
                                                    break;
                                                }
                                            }
                                            // Prioridade 2: Match parcial se nome >= 4 chars
                                            if (!foundJid && nameToSearch.length >= 4) {
                                                for (const p of participants) {
                                                    const contact = storeContacts[p.id] || {};
                                                    const pName = normalize(contact.name || contact.notify || "");
                                                    
                                                    if (pName.includes(nameToSearch) || nameToSearch.includes(pName)) {
                                                        foundJid = p.id;
                                                        break;
                                                    }
                                                }
