...
function markFailure(failedKey, force = false) {
  ...
  try {
    ...
    if (cfg.keys && Array.isArray(cfg.keys)) {
      ...
      saveConfig(cfg);
      console.log(chalk.yellow(`[AVISO] apiKeyManager: chave OpenRouter removida permanentemente por solicitação: ${failedKey.substring(0, 8)}...`));
    }
  } catch (e) {
...
function addKey(newKey) {
  ...
  try {
    ...
    saveConfig(cfg);
    console.log(chalk.green(`[INFO] apiKeyManager: Nova chave ${isClaude ? 'Claude' : 'OpenRouter'} adicionada com sucesso.`));
  } catch (e) {
